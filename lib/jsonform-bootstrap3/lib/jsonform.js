...
formNode.prototype.getFormValues = function (updateArrayPath) {
  ...
  for (var i = 0; i < formArray.length; i++) {
    ...
    var arrayMatchName = null;
    var arrayMatchValue = null;
    var cval = null;
    ...
    if (eltSchema._jsonform_checkboxes_as_array) {
      arrayMatchValue = formArray[i].value ? formArray[i].value.match(/\[([0-9]*)\]$/) : false;
      arrayMatchName  = name.match(/\[([0-9]*)\]$/);
      
      if (arrayMatchName || arrayMatchValue) {
        name = name.replace(/\[([0-9]*)\]$/, '');
        cval = jsonform.util.getObjKey(values, name) || [];
        if (arrayMatchValue) {
          // Value selected, push the corresponding enumeration item
          // to the data result
          cval.push(eltSchema['enum'][parseInt(arrayMatchValue[1],10)]);
        }
        else if (formArray[i].value === '1') {
          // Value selected, push the corresponding enumeration item
          // to the data result
          cval.push(eltSchema['enum'][parseInt(arrayMatchName[1],10)]);
        }
        else if (formArray[i].value) {
          // Added by multi-select with canadd = YES
